...
class Stocker():
    ...
    def __init__(self, ticker, nomini, periodski, intervalski, exchange='WIKI' ):
        ...
        try:
            #stock = quandl.get('%s/%s'(exchange, ticker), end_date="2005-12-31")
            stock = fix.download(  # or pdr.get_data_yahoo(...
        ...
        #stock = pd.read_csv('USDCAD60.csv')

        # Set the index to a column called Date
        stock = stock.reset_index(level=0)

        # Columns required for prophet
        stock['ds'] = pd.to_datetime(stock['Date'])
        ...
        self.stock.to_csv("stock_data.csv")
        ...
        self.predict_period = 365
        self.predict_freq = "D"
        self.numbers_changepoints = 20
    ...
    def plot_stock(self, start_date=None, end_date=None, stats=['Adj. Close'], plot_type='basic'):
        ...
        for i, stat in enumerate(stats):
            ...
            if plot_type == 'pct':
                ...
                plt.xlabel('Date'); plt.ylabel('Change Relative to Average (%)'); plt.title('%s Stock History' % self.nomini); 
    ...
    def changepoint_prior_analysis(self, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        train = self.stock[(self.stock['Date'] > (max(self.stock['Date']) - pd.DateOffset(months = self.training_months)))]
    ...
    def create_prophet_model(self, days=0, resample=False):
        ...
        stock_history = self.stock[self.stock['Date'] > (self.max_date - pd.DateOffset(months = self.training_months))]
        
        stock_history = self.remove_weekends(stock_history)
        ...
        ax.plot(stock_history['ds'], stock_history['y'], 'ko-', linewidth = 1.1, alpha = 0.5, ms = 0.8, label = 'Observations')
        
        # Plot the predicted values
        ax.plot(future['ds'], future['yhat'], 'forestgreen',linewidth = 1.4, label = 'Modeled');

        # Plot the uncertainty interval as ribbon
        ax.fill_between(future['ds'].dt.to_pydatetime(), future['yhat_upper'], future['yhat_lower'], alpha = 0.1, 
                       facecolor = 'y', edgecolor = 'k', linewidth = 1.4, label = 'Confidence Interval')
    ...
    def evaluate_prediction(self, start_date=None, end_date=None, nshares = None):
        ...
        if start_date is None:
            start_date = self.max_date - pd.DateOffset(months=12)
        if end_date is None:
            end_date = self.max_date
        ...
        train = self.stock[(self.stock['Date'] < start_date) & 
                           (self.stock['Date'] > (start_date - pd.DateOffset(months = self.training_months)))]
        ...
        if not nshares:
            ...
            print('The actual value was within the {:d}% confidence interval {:.2f}% of the time.\n'.format(int(100 * model.interval_width), in_range_accuracy))
            print("The R2 score on the Train set is:\t{:0.3f}".format(r2_score(train['y'], train['yhat'])))
            print("The R2 score on the Test set is:\t{:0.3f} \n".format(r2_score(test['y'], test['yhat'])))
            print("The Mean Absolute Percentage Error is:\t{:0.2f}% \n".format(np.mean(np.abs((test['y'] - test['yhat']) / test['y'])) * 100))
            ...
            # Plot the actual values
            ax.plot(train['ds'], train['y'], 'ko-', linewidth = 1.1, alpha = 0.6, ms = 0.8, label = 'Train Observations')
            ax.plot(test['ds'], test['y'], 'ko-', linewidth = 1.1, alpha = 0.6, ms = 0.8, label = 'Test Observations')
            
            # Plot the predicted values
            ax.plot(future['ds'], future['yhat'], 'forestgreen', linewidth = 1.1, label = 'Predicted');
        ...
        # If a number of shares is specified, play the game
            ...
            test_pred_decrease = test[test['pred_diff'] < 0]
            
            test_pred_increase.reset_index(inplace=True)
            test_pred_decrease.reset_index(inplace=True)

            prediction_profit = []
            prediction_strategy_profit = []
            test_pred_strategy = []
            ...
            for i, correct in enumerate(test_pred_increase['correct']):
                ...
                # If we predicted up and the price goes up, we gain the difference + STRATEGY
                if correct == 1:
                    prediction_profit.append(nshares * test_pred_increase.loc[i, 'real_diff'])
                    prediction_strategy_profit.append(nshares * test_pred_increase.loc[i, 'real_diff'])
             ...
             # STRATEGY Iterate through all the predictions and calculate profit from playing 
            for i, correct in enumerate(test_pred_decrease['correct']):
                
                # If we predicted up and the price goes down, we gain the difference
                if correct == 1:
                    prediction_strategy_profit.append(nshares * test_pred_decrease.loc[i, 'real_diff'])
                # If we predicted up and the price goes up, we lose the difference
                else:
                    prediction_strategy_profit.append(nshares * test_pred_decrease.loc[i, 'real_diff'])
                    
            #print(prediction_strategy_profit)          
            
            test_pred_increase['pred_profit'] = prediction_profit
            #test_pred_strategy =  prediction_strategy_profit.loc[:]
            ...
            #test = pd.merge(test, test_pred_strategy[['ds', [0]]], on = 'ds', how = 'left')
            #test.loc[0, 'strategy_profit'] = 0
            ...
            plt.text(x = text_location, 
                     y =  final_profit + (final_profit / 60),
                     s = '$%d' % final_profit,
                    color = 'g' if final_profit > 0 else 'r',
                    size = 18)
            
            plt.text(x = text_location, 
                     y =  final_smart + (final_smart / 60),
                     s = '$%d' % final_smart,
    ...
    def changepoint_date_analysis(self, search=None):
        ...
        c_data = c_data[:self.numbers_changepoints]
        ...
        if not search:
            ...
            # Set up line plot 
            plt.plot(train['ds'], train['y'], 'ko', ms = 2, label = 'Stock Price')
            plt.plot(future['ds'], future['yhat'], color = 'forestgreen', linewidth = 1.0, label = 'Modeled')
    ...
    def changepoint_prior_validation(self, start_date=None, end_date=None,changepoint_priors = [0.001, 0.05, 0.1, 0.2]):
        ...
        train = self.stock[(self.stock['Date'] > (start_date - pd.DateOffset(months = self.training_months))) & 
